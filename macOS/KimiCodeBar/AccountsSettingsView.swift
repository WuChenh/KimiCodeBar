...
struct AccountsSettingsView: View {
    ...
    @State private var apiKeyProvider: AccountProvider = .kimi
    ...
    var body: some View {
        ...
        .alert(LanguageManager.tr("修改 API Key"), isPresented: $showEditKeyAlert) {
            SecureField(
                editingKeyAccount?.provider == .deepseek
                    ? LanguageManager.tr("sk-...")
                    : LanguageManager.tr("sk-kimi-..."),
                text: $editKeyInput
            )
            Button(LanguageManager.tr("保存")) {
    ...
    private var methodChooser: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 10) {
                SettingsOptionCard(
                    title: "DeepSeek",
                    subtitle: languageManager.tr("DeepSeek API Key 登录"),
                    iconName: "d.circle",
                    isSelected: false
                ) {
                    showsMethodChooser = false
    ...
    private var apiKeyForm: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 10) {
                Text(apiKeyProvider == .deepseek
                    ? languageManager.tr("DeepSeek API Key 登录")
                    : languageManager.tr("API Key 登录"))
                    .font(.system(size: 14, weight: .medium))
                    .foregroundStyle(.kimiTextPrimary)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 6) {
                        ...
                        if provider == .deepseek {
                            StatusTag(text: "DeepSeek", color: .kimiTextSecondary)
                        }
