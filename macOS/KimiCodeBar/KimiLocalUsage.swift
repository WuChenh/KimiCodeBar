      ...
      autoreleasepool {
        ...
        text.enumerateLines { line, _ in
          guard line.contains("\"usage.record\""),
            ...
            let timeMs = (event["time"] as? NSNumber)?.doubleValue
          else { return }
...
struct LocalUsageCard: View {
  ...
  private var chartDays: [LocalUsageDay] {
    ...
    let calendar = Calendar.current
    let start =
      calendar.date(byAdding: .day, value: -29, to: calendar.startOfDay(for: Date()))
      ?? .distantPast
    ...
    HStack(spacing: 2) {
      ForEach(LocalUsageRange.allCases) { item in
        ...
        Text(item.displayName)
          ...
          .foregroundStyle(
            isSelected ? Color.white : (isHovered ? Color.kimiTextPrimary : Color.kimiTextSecondary)
          )
          ...
          .background(
            Capsule()
              .fill(
                isSelected ? Color.kimiBlue : Color.kimiTextPrimary.opacity(isHovered ? 0.08 : 0))
          )
...
extension String {
  fileprivate func relativeTo(_ base: String) -> String? {
  ...
  func body(content: Content) -> some View {
    content.overlay(
      GeometryReader { geo in
        ...
        .offset(
          x: (phase + 1) / 2 * (geo.size.width + geo.size.width * 0.5) - geo.size.width * 0.25
        )
