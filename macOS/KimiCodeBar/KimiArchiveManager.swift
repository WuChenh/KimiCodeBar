...
final class KimiArchiveManager: ObservableObject {
  ...
  private init() {
    autoArchiveEnabled =
      UserDefaults.standard.object(forKey: "autoArchiveEnabled") as? Bool ?? false
  ...
  nonisolated private static func scanSessionsInBackground() -> (
    sessions: [KimiSession], error: String?
  ) {
  ...
  nonisolated static func relativeTimeString(from date: Date) -> String {
    ...
    formatter.locale = Locale(
      identifier: LanguageManager.resolvedLanguage == .zhHans ? "zh_CN" : "en_US")
