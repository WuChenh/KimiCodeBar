...
final class KimiCodeBarQuotaService {
  ...
  private func parse(_ data: Data) -> KimiQuota? {
    ...
    let boosterWallet: BoosterWallet? = {
      ...
      let currency =
        raw.monthlyChargeLimit?.currency
  ...
  private func makeDetail(limit: String?, used: String?, remaining: String?, resetTime: String?)
    -> QuotaDetail
  {
    ...
    return QuotaDetail(
      used: us, limit: li, remaining: re, resetTime: parseDate(resetTime), percentage: pct)
  }
...
extension QuotaDetail {
  var timeUntilReset: String {
    ...
    let components = Calendar.current.dateComponents(
      [.day, .hour, .minute], from: now, to: resetTime)
