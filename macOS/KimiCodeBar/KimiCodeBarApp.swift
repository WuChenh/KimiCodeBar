...
// MARK: - Kimi Code Logo（纯 SwiftUI）

/// Kimi Code Logo：纯 SwiftUI 实现（无 AppKit/CALayer 依赖）。
/// 配色风格：filled 蓝底实色（默认，用于常规底色）；outline 白色描边（用于彩色渐变背景）。
struct AnimatedKimiCodeLogo: View {
    var width: CGFloat = 44
    let isAnimating: Bool
    var style: LogoStyle = .filled
    ...
    var body: some View {
        ...
        ZStack {
            RoundedRectangle(cornerRadius: bodyR)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: bodyR)
                        .stroke(bodyStrokeColor, lineWidth: strokeWidth)
                )
                .shadow(color: bodyShadowColor, radius: 8 * scale, x: 0, y: -3 * scale)
                .frame(width: bodyW, height: bodyH)

            KimiCodeEye(scale: scale, isAnimating: isAnimating, style: style)
    ...
    private var bodyStrokeColor: Color {
        ...
        case .outline: return Color.white.opacity(0.9)
        }
    }

    private var bodyShadowColor: Color {
        switch style {
        case .filled: return .kimiBlue.opacity(0.35)
        case .outline: return .clear
        }
    }
    ...
    private var strokeWidth: CGFloat {
        ...
        case .outline: return max(1.2, 1.2 * scale)
        }
    }
}
...
private struct KimiCodeEye: View {
    let scale: CGFloat
    let isAnimating: Bool
    var style: AnimatedKimiCodeLogo.LogoStyle

    /// 动画活跃标志：onAppear / isAnimating 开启时置 true，onDisappear / isAnimating
    /// 关闭时置 false。调度闭包中读取的是 State 最新值（引用语义），
    /// 视图销毁后调度链立即停止，不会递归空转。
    ...
    private var eyeColor: Color {
        switch style {
        case .filled:
            // 动态眼白：跟随系统浅色/深色外观（filled 蓝底上反色）
            return dynamicColor(
                light: .white,
                dark: NSColor(red: 0x18 / 255.0, green: 0x18 / 255.0, blue: 0x17 / 255.0, alpha: 1.0)
            )
        case .outline:
...
struct KimiMenu: View {
    ...
    private let deepseekConsoleURL = URL(string: "https://platform.deepseek.com/api_keys")!
    private let githubURL = URL(string: "https://github.com/xifandev/KimiCodeBar")!

    /// 主账号所属平台的「控制台」入口（DeepSeek 主账号时跳 DeepSeek 平台）
    private var primaryConsoleURL: URL {
        if model.primaryAccount?.provider == .deepseek {
            return deepseekConsoleURL
        }
        return consoleURL
    }

    /// 控制台按钮的文字图标：跟随主账号平台（KIMI / DS）
    private var primaryProviderTextIcon: String {
        if model.primaryAccount?.provider == .deepseek {
            return "DS"
        }
        return "KIMI"
        ...
        VStack(spacing: 14) {
            ...
            HStack(spacing: 8) {
                ActionButton(
                    ...
                    textIcon: primaryProviderTextIcon,
                    action: {
                        dismissMenuBarPanel()
                        NSWorkspace.shared.open(primaryConsoleURL)
                    }
                )
...
/// DeepSeek 账号的余额大卡片（单账号时使用）：大数字展示总余额，
/// 下方两列展示赠送余额与充值余额。
private struct DeepSeekBalanceCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                if let balance = balance, !isLoading {
                    Text(balance.isAvailable ? LanguageManager.tr("可用") : LanguageManager.tr("不可用"))
                        .font(.system(size: 10, weight: .medium))
                        .foregroundStyle(balance.isAvailable ? .green : .kimiTextTertiary)
                        .padding(.horizontal, 6)
                        .padding(.vertical, 2)
                        .background((balance.isAvailable ? Color.green : Color.kimiTextTertiary).opacity(0.12))
                        .clipShape(RoundedRectangle(cornerRadius: 4))
                }
            }
            ...
            ZStack(alignment: .leading) {
                if !isLoading {
                    Text(currencyText(balance?.totalBalance ?? 0, currency: balance?.currency ?? "CNY"))
                        .font(.system(size: 32, weight: .bold, design: .rounded))
                        .monospacedDigit()
                        .foregroundStyle(.kimiTextPrimary)
                        .transition(.opacity.animation(.easeInOut(duration: 0.25)))
            ...
            if let balance = balance, !isLoading {
                HStack(spacing: 16) {
                    balanceDetailRow(
                        ...
                        value: formatCurrency(balance.grantedBalance, currency: balance.currency)
                    )
                    balanceDetailRow(
                        label: LanguageManager.tr("充值余额"),
                        value: formatCurrency(balance.toppedUpBalance, currency: balance.currency)
                    )
                    Spacer(minLength: 0)
                }
                .transition(.opacity.animation(.easeInOut(duration: 0.25)))
    ...
    /// 余额展示：带货币符号，最多保留两位小数（如 ¥120 / ¥12.5 / ¥0.75）
    private func currencyText(_ amount: Double, currency: String) -> String {
        let symbol: String
...
/// 多账号列表里 DeepSeek 账号的紧凑余额行：账号名 + 余额数字，信息层级与 AccountQuotaCard 对齐。
private struct CompactDeepSeekBalanceRow: View {
    ...
    private var balance: DeepSeekBalance? {
        model.deepseekBalances[account.id]
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 6) {
                ...
                tagPill("DeepSeek", color: .kimiTextSecondary)
            ...
            if case .unauthorized = state {
                ...
                if let balance = balance, !isLoadingState {
                    HStack(alignment: .firstTextBaseline, spacing: 8) {
                        Text(formatCurrency(balance.totalBalance, currency: balance.currency))
                        ...
                        VStack(alignment: .trailing, spacing: 2) {
                            balanceMiniRow(LanguageManager.tr("赠送"), balance.grantedBalance, balance.currency)
                            balanceMiniRow(LanguageManager.tr("充值"), balance.toppedUpBalance, balance.currency)
                        }
                    }
    ...
    private func balanceMiniRow(_ label: String, _ value: Double, _ currency: String) -> some View {
        HStack(spacing: 4) {
            Text(label)
                .font(.system(size: 11))
                .foregroundStyle(.kimiTextSecondary)
    ...
    /// 卡片头部的小号标签：与 AccountQuotaCard.tagPill 同一比例
        ...
        VStack(spacing: 8) {
            if model.accounts.count <= 1, let account = model.accounts.first {
                if account.provider == .deepseek {
                    DeepSeekBalanceCard(
                        balance: model.deepseekBalances[account.id],
                        isLoading: (model.accountStates[account.id] ?? .idle) == .loading
                    )
                } else {
...
final class KimiCodeBarModel: ObservableObject {
    ...
    /// 每个 DeepSeek 账号最近一次拉取成功的余额（失败时保留旧值）
    @Published var deepseekBalances: [UUID: DeepSeekBalance] = [:]
    ...
    func refreshAllAccounts(showsLoading: Bool = true) {
        ...
        Task {
            ...
            var results: [(UUID, KimiQuota?, DeepSeekBalance?, QuotaError?)] = []
            await withTaskGroup(of: (UUID, KimiQuota?, DeepSeekBalance?, QuotaError?).self) { group in
                for account in snapshot.accounts {
                    group.addTask {
                        // 按平台分派：Kimi 走 usages 配额接口，DeepSeek 走 balance 余额接口。
                        // 扩展新平台（如 DeepSeek）时在此按 account.provider 分派对应平台的配额服务。
                        if account.provider == .deepseek {
                            guard case .apiKey(let key) = account.credential else {
                                return (account.id, nil, nil, nil)
                            }
                            switch await self.deepseekService.fetchBalance(apiKey: key) {
                            case .success(let balance):
                                return (account.id, nil, balance, nil)
                            case .failure(let error):
                                // Key 无效（401/403）按「登录失效」处理，引导用户修改 Key
                                if case .httpError(let statusCode, _) = error, statusCode == 401 || statusCode == 403 {
                                    return (account.id, nil, nil, nil)
                                }
                                return (account.id, nil, nil, error)
                            }
                        }
                        ...
                        case .oauth:
                            ...
                            case .success(let quota):
                                return (account.id, quota, nil, nil)
                            case .failure(let error):
                                return (account.id, nil, nil, error)
                            }
                        case .apiKey(let key):
            ...
            await MainActor.run {
                for (id, quota, balance, error) in results {
                    if let quota {
                        self.accountQuotas[id] = quota
                        self.accountStates[id] = .loaded
                    } else if let balance {
                        self.deepseekBalances[id] = balance
                        self.accountStates[id] = .loaded
                    } else if let error {
                        self.accountStates[id] = .failed(
                            snapshot.accounts.first(where: { $0.id == id })?.provider == .deepseek
                                ? self.deepseekErrorDescription(error)
                ...
                self.deepseekBalances = self.deepseekBalances.filter { validIDs.contains($0.key) }
    ...
    private func syncPrimaryCompat() {
        ...
        // DeepSeek 主账号：菜单栏只显示余额文本（无周/5小时配额图形）
        let primaryAccount = accounts.first(where: { $0.id == primaryID })
        if primaryAccount?.provider == .deepseek {
            quota = nil
            if let balance = deepseekBalances[primaryID] {
                text = LanguageManager.tr("DS %@", arguments: [balance.shortText])
            } else {
                text = "DS --"
            }
        } else if let primaryQuota = accountQuotas[primaryID] {
            quota = primaryQuota
    ...
    func removeAccount(_ id: UUID) {
        ...
        deepseekBalances.removeValue(forKey: id)
    ...
    /// 添加 API Key 账号：先拉一次 usages（Kimi）或 balance（DeepSeek）验证 Key 并提取账号标识去重，成功则落库。
    /// 返回 nil 表示成功；否则返回面向用户的错误文案。
    @discardableResult
    func addApiKeyAccount(key: String, alias: String?, provider: AccountProvider = .kimi) async -> String? {
        ...
        if provider == .deepseek {
            ...
            case .success:
                ...
                store.addAccount(KimiAccount(
                    id: UUID(),
                    alias: (alias?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
                        ? alias?.trimmingCharacters(in: .whitespacesAndNewlines) : nil,
                    provider: .deepseek,
                ...
                return nil
            case .failure(let error):
                return deepseekErrorDescription(error)
            }
        }
    ...
    /// DeepSeek 平台错误文案（与 Kimi 分开，避免误导平台归属）。
    private func deepseekErrorDescription(_ error: QuotaError) -> String {
        switch error {
        case .invalidKeyFormat:
            return LanguageManager.tr("API Key 格式错误")
        case .invalidURL:
            return LanguageManager.tr("请求地址无效")
        case .networkError(let msg):
            return LanguageManager.tr("网络错误：%@", arguments: [msg])
        case .httpError(let code, let msg):
            return LanguageManager.tr("DeepSeek API 返回错误（%1$@）：%2$@", arguments: ["\(code)", msg])
        case .invalidResponse:
            return LanguageManager.tr("无法解析 API 返回数据")
        }
    }
