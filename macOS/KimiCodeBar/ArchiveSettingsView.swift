...
struct ArchiveSettingsView: View {
  ...
  private var eligibleCount: Int {
    manager.sessions.filter {
      !$0.isArchived
        && now.timeIntervalSince($0.updatedAt) > manager.autoArchiveThreshold.timeInterval
    }.count
  }
        ...
        SettingsCard {
          VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 12) {
              ...
              ArchiveActionButton(
                title: eligibleCount > 0
                  ? languageManager.tr("立即归档 %d 个会话", eligibleCount)
                  : languageManager.tr("暂无符合条件的会话"),
                icon: "archivebox",
                disabled: eligibleCount == 0 || manager.isScanning
              ) {
                Task {
                  let count = await manager.archiveAllEligible(
            ...
            if let date = manager.lastAutoArchiveDate {
              ...
              HStack {
                LText(
                  "上次自动归档：%1$@，共 %2$d 个会话", KimiArchiveManager.relativeTimeString(from: date),
                  manager.lastAutoArchiveCount
                )
                .font(.system(size: 12))
                  ...
                  if isExpanded {
                    ForEach(group.sessions) { session in
                      SessionRow(
                        session: session,
                        threshold: manager.autoArchiveThreshold,
                        isEligible: !session.isArchived
                          && now.timeIntervalSince(session.updatedAt)
                            > manager.autoArchiveThreshold.timeInterval
                      ) {
    ...
    Button(action: action) {
      ...
      .background(
        disabled
          ...
          : (isHovered ? Color.kimiTextPrimary.opacity(0.10) : Color.kimiTextPrimary.opacity(0.06))
      )
    ...
    Button(action: onToggle) {
      HStack(spacing: 8) {
        ...
        Text(name)
          ...
          .foregroundStyle(isHovered ? .kimiTextPrimary : .kimiTextPrimary)
  ...
  var body: some View {
    Button(action: action) {
      LText("取消归档")
        ...
        .padding(.vertical, 5)
        .background(
          isHovered ? Color.kimiTextPrimary.opacity(0.14) : Color.kimiTextPrimary.opacity(0.08)
        )
